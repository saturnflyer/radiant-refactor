...
module Radiant
  class Page < ActiveRecord::Base
    ...
    has_many :fields, :class_name => 'Radiant::PageField', :order => 'id', :dependent => :destroy
    ...
    def published?
      status == Radiant::Status[:published]
    end
  
    def scheduled?
      status == Radiant::Status[:scheduled]
    end
    ...
    def find_by_path(path, live = true, clean = true)
      ...
      if (my_path == path) && (not live or published?)
        ...
        condition = "status_id = #{Radiant::Status[:published].id} and (#{condition})" if live
    ...
    def update_status
      ...
      if self.published_at != nil && (published? || scheduled?)
        self[:status_id] = Radiant::Status[:scheduled].id if self.published_at  > Time.zone.now
        self[:status_id] = Radiant::Status[:published].id if self.published_at <= Time.zone.now
      end
    ...
    class << self
      ...
      def load_subclasses
        ([Rails.root] + Radiant::Extension.descendants.map(&:root)).each do |path|
      ...
      def new_with_defaults(config = Radiant::Config)
        ...
        page.status = Radiant::Status[default_status] if default_status
        page
      end
      ...
      private
        ...
        def default_page_parts(config = Radiant::Config)
          ...
          default_parts.map do |name|
            Radiant::PagePart.new(:name => name, :filter_id => config['defaults.page.filter'])
          end
        end
        ...
        def default_page_fields(config = Radiant::Config)
          ...
          default_fields.map do |name|
            Radiant::PageField.new(:name => name)
          end
        end
    end
    ...
    private
      ...
      def lazy_initialize_parser_and_context
        unless @parser and @context
          @context = Radiant::PageContext.new(self)
