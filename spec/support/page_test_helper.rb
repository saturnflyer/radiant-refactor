class NoCachePage < Radiant::Page
...
unless defined?(::CustomFileNotFoundPage)
  class ::CustomFileNotFoundPage < Radiant::FileNotFoundPage
  end
end

class TestPage < Radiant::Page
...
class VirtualPage < Radiant::Page
  def virtual?
    true
  end
end
...
module PageTestHelper
  ...
  def get_test_page(title = @page_title)
    Radiant::Page.find_by_title(title)
  end
  
  def create_test_page(options = {})
    options[:title] ||= @page_title
    klass = options.delete(:class_name) || Radiant::Page
